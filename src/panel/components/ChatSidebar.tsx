...
const ChatSidebar: React.FC<ChatSidebarProps> = ({
  ...
  const visibleChannels = currentChannels.filter((c) => {
    ...
    return channelObj.tags.some((channelTag: Tag) => 
    ...
    <div className="w-60 bg-gray-900 flex flex-col h-full border-r border-gray-700">
      ...
      <div className="h-12 flex items-center justify-between px-4 shadow-sm border-b border-gray-700 hover:bg-gray-800 transition-colors duration-200 group/header">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover/header:opacity-100 transition-opacity duration-200">
            <button onClick={onEditGroup} className="text-gray-500 hover:text-gray-200 transition-all duration-200 p-1.5 rounded-md hover:bg-gray-700">
            ...
            {canDeleteGroup && (
            <button onClick={onTrashClick} className="text-gray-500 hover:text-red-500 transition-all duration-200 p-1.5 rounded-md hover:bg-gray-700">
          ...
          return (
            <div key={c} className={`group flex justify-between items-center px-2 py-1.5 rounded-md mx-1 cursor-pointer transition-all duration-200 select-none ${isActive ? 'bg-gray-800 text-purple-400' : 'text-gray-400 hover:bg-gray-800/50 hover:text-purple-400'}`} onClick={() => { if (currentChannel !== c) { loadChannel(c); }}}>
              <div className="flex items-center flex-1 min-w-0 overflow-hidden">
                ...
                <span className="truncate text-sm font-medium transition-colors">{formatChannelName(c).replace('#', '')}</span>
              </div>
              
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <button onClick={(e) => { e.stopPropagation(); toggleMuteChannel(fullKey); }} className={`p-1 rounded hover:bg-gray-700 transition-colors ${isMuted ? 'text-red-400' : 'text-gray-400 hover:text-yellow-400'}`} title={isMuted ? "activar sonido" : "silenciar"}>
                    {isMuted ? (
                  ...
                  {cId && (
                    <>
                      {canEdit && (
                      <button onClick={(e) => { e.stopPropagation(); if (channelObj) setChannelToEdit(channelObj); }} className="p-1 text-gray-400 hover:text-blue-400 hover:bg-gray-700 rounded transition-colors" title="Configurar canal">
                      ...
                      {canDelete && (
                      <button onClick={(e) => { e.stopPropagation(); handleDelete(cId, c); }} className="p-1 text-gray-400 hover:text-red-500 hover:bg-gray-700 rounded transition-colors" title="eliminar">
      ...
      {canCreate && (
        <button onClick={handleCreate} title="Crear un nuevo canal" className="w-full flex items-center justify-center gap-2 px-2 py-2 text-gray-400 hover:text-white hover:bg-purple-600/20 hover:border-purple-500/50 border border-dashed border-gray-700 rounded-md transition-all duration-200 text-sm font-medium group">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-500 group-hover:text-purple-400 transition-colors"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
          <span>Crear Canal</span>
        </button>
        )}
      </div>

      {channelToEdit && <ChannelSettingsModal isOpen={!!channelToEdit} channel={channelToEdit} onClose={() => setChannelToEdit(null)} />}
    </div>
  );
};
